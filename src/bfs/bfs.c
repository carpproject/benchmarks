...
int main( int argc, char *argv[] )
{
  ...
  int *edge_count = (int *)pencil_alloc(sizeof(int)*no_of_nodes);        //no of edges, vertex i has.
  ...
  int *dst_node_index = (int *)pencil_alloc(sizeof(int) * no_of_edges);
  int *cost           = (int *)pencil_alloc(sizeof(int) * no_of_edges);
  int *cost_ref       = (int *)malloc(sizeof(int) * no_of_edges);
  char *front          = (char *)pencil_alloc(sizeof(char) * no_of_nodes);
  char *updating_front = (char *)pencil_alloc(sizeof(char) * no_of_nodes);
  char *visited        = (char *)pencil_alloc(sizeof(char) * no_of_nodes);
  ...
  pencil_free(edge_start_no);
  pencil_free(edge_count);
  pencil_free(dst_node_index);
  pencil_free(cost);
  free(cost_ref);
  pencil_free(front);
  pencil_free(updating_front);
  pencil_free(visited);
  pencil_shutdown();
  return ret;
}
